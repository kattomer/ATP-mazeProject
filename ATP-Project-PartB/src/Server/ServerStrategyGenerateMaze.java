...
public class ServerStrategyGenerateMaze implements IServerStrategy {
    ...
    public void serverStrategy(InputStream inFromClient, OutputStream outToClient) {
        try (ObjectInputStream fromClient = new ObjectInputStream(inFromClient);
            ...
            if (Configurations.getInstance().getMazeGenerating().equals("SimpleMazeGenerator")) {
                generator = new SimpleMazeGenerator();
            } else if (Configurations.getInstance().getMazeGenerating().equals("EmptyMazeGenerator")) {
